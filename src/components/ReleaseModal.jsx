...
export default function ReleaseModal({ releases }) {
  const getReleaseId = useCallback(release => release.id, []);
  const { selected: activeRelease, clearSelection } = useHashSelection(releases, getReleaseId);
  useBodyScrollLock(Boolean(activeRelease));

  useEffect(() => {
    if (!activeRelease || typeof window === 'undefined' || typeof document === 'undefined') {
      return undefined;
    }
  ...
  return (
    <ModalFrame
      isOpen={Boolean(activeRelease)}
      onClose={clearSelection}
      panelClassName="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-[2.5rem] border border-white/60 bg-white p-6 shadow-[0_40px_90px_rgba(15,23,42,0.25)] sm:p-10"
      labelledBy="release-modal-title"
    >
      <div className="grid gap-8 lg:grid-cols-[minmax(0,320px)_1fr] lg:gap-12">
        <img
          src={activeRelease.cover}
          alt={activeRelease.title}
          className="w-full rounded-[1.75rem] border border-primary/10 bg-white/80 p-4 shadow-lg"
          loading="lazy"
          decoding="async"
        />
        <div className="flex flex-col gap-4">
          <h2 id="release-modal-title" className="text-2xl font-bold font-heading text-text sm:text-3xl">
            {activeRelease.title}
          </h2>
          <p className="text-base text-text/80 sm:text-lg">
            {activeRelease.year} &bull; {activeRelease.type}
          </p>
